...
let buildAction (resolver:IResolver) (act:MethodInfo) handlerType  =
  if not (resolver.CanResolve handlerType) then raise (ResolutionFailed handlerType)
  let handler = <@ resolver.Get handlerType @>
...
let findHandlers(catalog:ConcurrentDictionary<_,_>) extractor msgType =
    let msgType = if FSharpType.IsUnion msgType then msgType.BaseType else msgType
    try
